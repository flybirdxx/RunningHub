# feature:detail:presentation Instructions

## Responsibility

本模块承载 `detail` 的 Presentation 契约和可复用状态逻辑，例如 UiState、Action、StateHolder、Interactor、Coordinator 或 ScreenModel 门面。
...
- 详情页不得直接读取 API Key；上传和任务执行通过 task domain 仓库。
