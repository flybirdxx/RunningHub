# feature:discovery:domain Instructions

## Responsibility

本模块只定义 `discovery` 的领域模型、Repository interface、UseCase、业务错误和业务规则。
...
- 目录查询使用 `CatalogQuery`、`CatalogSort`、`CatalogTagRange` 和 `CatalogError` 等领域类型。
...
./gradlew :feature:discovery:domain:allTests
```
