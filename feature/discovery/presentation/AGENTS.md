# feature:discovery:presentation Instructions

## Responsibility

本模块承载 `discovery` 的 Presentation 契约和可复用状态逻辑，例如 UiState、Action、StateHolder、Interactor、Coordinator 或 ScreenModel 门面。
...
- 主列表、搜索、分类和排序必须使用请求版本或等价机制隔离旧响应。
- 分页追加按 WebApp.id 去重。
