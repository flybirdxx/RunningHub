# feature:task:domain Instructions

## Responsibility

本模块只定义 `task` 的领域模型、Repository interface、UseCase、业务错误和业务规则。
...
- 任务状态、历史来源、输出类型、上传结果必须使用稳定领域模型；轮询终态必须明确。
...
./gradlew :feature:task:domain:allTests
```
