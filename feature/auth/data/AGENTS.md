# feature:auth:data Instructions

## Responsibility

本模块实现 `feature:auth:domain` 中的仓库接口，并负责远端 API、DTO、Mapper、Data DI 和数据层错误映射。
...
- 不直接导航；登录/注销/401 只通过 `SessionManager` 状态传播。
- 凭据只能通过 `CredentialStore` 读写；不能记录 token、Cookie、验证码 token。
- TAC/captcha token 只参与下一次短信发送请求，不持久化。
