# feature:auth:domain Instructions

## Responsibility

本模块只定义 `auth` 的领域模型、Repository interface、UseCase、业务错误和业务规则。
...
- `SessionManager` 是会话状态唯一事实来源；Domain 不携带 Token 值和最终 UI 文案。
- 短信验证码、图形验证码、登录失败等错误必须使用稳定错误类型。
