 ...
 * 本类不依赖 composeApp、Composable 或平台 API，因此可以在 feature presentation 模块内独立测试。
 *
 * @param projectRepository 快捷创作项目仓库接口，提供项目列表、详情和项目变更能力。
 ...
 */
class QuickCreateProjectStateHolder(
    ...
    fun loadProjects() {
        scope.launch {
            ...
            val projects = projectRepository.listQuickCreationProjects(page = 1, size = 20)
            projects.fold(
                onSuccess = { page ->
    ...
    fun loadMoreProjects() {
        ...
        scope.launch {
            ...
            val projects = projectRepository.listQuickCreationProjects(page = nextPage, size = 20)
            projects.fold(
                onSuccess = { page ->
    ...
    fun toggleProjectPin(projectId: String) {
        ...
        scope.launch {
            ...
            val result = projectRepository.pinQuickCreationProject(projectId = projectId, pinned = targetPinned)
            result.fold(
                onSuccess = {
                    uiState.update { state ->
                        state.copy(
    ...
    fun createProject(name: String) {
        ...
        scope.launch {
            ...
            val result = projectRepository.createQuickCreationProject(trimmedName)
            result.fold(
                onSuccess = { project ->
                    uiState.update { state ->
                        state.copy(
    ...
    fun renameProject(projectId: String, name: String) {
        ...
        scope.launch {
            ...
            val result = projectRepository.renameQuickCreationProject(
                projectId = projectId,
                name = trimmedName,
            )
            result.fold(
    ...
    fun deleteProject(projectId: String) {
        ...
        scope.launch {
            ...
            val result = projectRepository.deleteQuickCreationProject(projectId)
            result.fold(
                onSuccess = {
                    uiState.update { state ->
                        state.copy(
    ...
    fun selectProjectDetail(projectId: String) {
        ...
        scope.launch {
            ...
            val detail = projectRepository.getQuickCreationProjectDetail(projectId)
            detail.fold(
                onSuccess = { project ->
                    uiState.update {
                        it.copy(
