# feature:quickcreate:presentation Instructions

## Responsibility

本模块承载 `quickcreate` 的 Presentation 契约和可复用状态逻辑，例如 UiState、Action、StateHolder、Interactor、Coordinator 或 ScreenModel 门面。
...
- ScreenModel 保持门面；新增流程优先进入 Coordinator、StateHolder 或 Interactor。
- 上传、计费、防抖、生成和轮询必须可取消且有测试。
