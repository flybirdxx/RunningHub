...
data class QuickCreateTaskQueryResponseDto(
    ...
    @SerialName("usage") val usage: QuickCreateTaskUsageDto? = null,
) : TaskResponse

/**
 * QuickCreate OpenAPI 任务查询返回的计费用量信息。
 *
 * 该对象只描述远端协议字段，不直接暴露给 Presentation；Repository 需要时再显式映射到
 * QuickCreate 领域模型。字段均为服务端字符串，保留原始精度和格式，避免客户端在 DTO 层
 * 提前做金额或耗时换算。
 *
 * @property consumeMoney 服务端返回的现金消耗金额字符串；`null` 表示当前响应未包含现金计费信息。
 * @property consumeCoins 服务端返回的积分/算力消耗字符串；`null` 表示当前响应未包含积分计费信息。
 * @property taskCostTime 服务端记录的任务耗时字符串，单位由远端协议决定；`null` 表示任务仍未产生耗时统计。
 * @property thirdPartyConsumeMoney 第三方模型侧成本字符串；`null` 表示服务端未返回或该任务不涉及第三方成本。
 */
@Serializable
data class QuickCreateTaskUsageDto(
