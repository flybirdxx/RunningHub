    ...
    if step_number == 3:
        return {
            "actions": [
                ...
                "",
                "EXPLORE each selected dimension sequentially: (no parallel agents in Gemini)",
                "",
                "FOR EACH DIMENSION:",
                ...
                "",
                "Complete all dimension explorations before proceeding.",
            ],
            "next": "After all dimensions explored, invoke step 8 with combined findings"
        }
    ...
    if step_number == 9:
        return {
            "actions": [
                "DEEP-DIVE DISPATCH",
                "",
                "VERIFY each candidate sequentially: (no parallel agents in Gemini)",
                "",
                "FOR EACH CANDIDATE:",
                ...
                "",
                "Complete all candidate verifications before proceeding.",
            ],
            "next": "After all candidates verified, invoke step 12 with all verdicts"
        }
    ...
    if step_number == 17:
        return {
            "actions": [
                "RECONCILE DISPATCH",
                "",
                "APPLY resolutions sequentially: (no parallel agents in Gemini)",
                "  For each resolution target:",
                "    1. Read target file",
                "    2. Apply resolution per user decision",
                "    3. Verify change applied correctly",
                "",
                "FOR EACH FILE IN CURRENT WAVE:",
                ...
                "",
                "Complete all wave files before proceeding.",
            ],
            "next": "After all wave files processed, invoke step 20 with results"
        }
...
def main():
    ...
    # Determine phase (Gemini CLI executes all steps sequentially in single agent)
    ...
    print(f"STEP {args.step_number}/{args.total_steps} [{phase}]")
    print()
    print("ACTIONS:")
