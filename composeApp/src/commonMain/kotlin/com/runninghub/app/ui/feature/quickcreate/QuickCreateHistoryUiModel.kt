 ...
 *
 * @property source 原始历史任务实体，来自 [QuickCreateHistoryStateHolder] 请求 Domain Repository 后的返回值。
 ...
 */
internal fun QuickCreationHistoryItem.toQuickCreateHistoryUiItem(): QuickCreateHistoryUiItem {
 ...
 */
internal fun QuickCreationHistoryItem.toQuickCreateHistoryDetailUiItem(): QuickCreateHistoryDetailUiItem {
    val primaryOutput = outputs.firstOrNull()?.toQuickCreateHistoryOutputUi()
...
private fun QuickCreationHistoryItem.toHistoryCashText(): String? =
    if (cashAmount > 0.0) {
        "${formatCashAmount(cashAmount)} ${cashCurrency.orEmpty()}"
    } else {
        null
    }
