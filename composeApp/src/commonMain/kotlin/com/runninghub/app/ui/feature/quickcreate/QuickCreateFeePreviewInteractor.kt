 ...
 * 它不持有 Compose 或平台类型，便于后续继续迁移到 feature/quickcreate/presentation 边界。
 *
 * 并发约束：
 ...
 *
 * @param quickCreateRepository 快捷创作仓库，负责执行图片和视频计费预览请求。
 ...
 */
internal class QuickCreateFeePreviewInteractor(
    ...
    private suspend fun previewVideo(
        ...
        quickCreateRepository.previewVideoQuickCreationFee(request).fold(
            onSuccess = { preview ->
                if (isCurrentFeePreviewRequest(requestSeq)) {
    ...
    private suspend fun previewImage(
        ...
        quickCreateRepository.previewImageQuickCreationFee(request).fold(
            onSuccess = { preview ->
                if (isCurrentFeePreviewRequest(requestSeq)) {
                    applyFeePreview(preview)
                }
