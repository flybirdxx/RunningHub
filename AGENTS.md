...
上述类型中，每一个主构造函数属性都必须使用以下任一方式写中文 KDoc：

1. 在类级 KDoc 中使用 `@property` 逐项说明；
2. 在属性声明前直接使用 `/** ... */`。

同一个属性不要同时使用两种方式重复说明。
任何未说明业务语义的属性，都视为注释不完整。

字段注释必须说明代码类型本身无法表达的信息。根据字段实际情况，至少覆盖：
...
- 空值语义：可空字段为 `null` 时具体代表什么，而不是只写“可以为空”。
- 生命周期：何时赋值、何时清空、何时失效、是否跨页面或跨进程保存。
 ...
 * 登录页面的完整可渲染状态。
 *
 * 状态由 [LoginScreenModel] 维护，只承载页面输入和交互状态；
 * 认证凭据由 [AuthRepository] 的实现层持久化，本对象不得保存 Token、Cookie 或 API Key。
 *
 * 状态不变量：当 [loginSuccess] 为 `true` 时，[user] 原则上必须非空。
 *
 * @property phone 用户当前输入的中国大陆手机号，不包含界面固定展示的 `+86` 前缀。
 * 当前校验规则为至少 11 位且全部为数字；空字符串表示用户尚未输入。
 * 该值来自用户输入，仅存在于当前页面状态中，不应写入日志。
 *
 * @property smsCode 用户当前输入的短信验证码。
 * ScreenModel 只保留数字并限制最多 6 位；提交登录时至少需要 4 位。
 * 空字符串表示尚未输入，或验证码错误后已按业务规则清空。
 *
 * @property password 用户当前输入的明文密码。
 * 仅用于本次密码登录请求，不得持久化、打印到日志或写入错误报告；
 * 空字符串表示尚未输入，切换到短信登录模式时该值可以继续保留在内存中。
 *
 * @property isSmsMode 当前登录方式。
 * `true` 表示短信验证码登录，页面展示验证码输入框；
 * `false` 表示密码登录，页面展示密码输入框。
 *
 * @property isLoading 是否正在提交登录请求。
 * `true` 时登录按钮应禁用并展示进度；`false` 表示当前没有进行中的登录请求。
 * 该字段不代表“发送验证码”流程，发送验证码由 [isSendingCode] 单独表示。
 *
 * @property isSendingCode 是否正在请求发送短信验证码。
 * `true` 时发送按钮应禁用，防止同一手机号重复提交；
 * 请求成功、失败或 ScreenModel 被销毁后恢复为 `false`。
 *
 * @property countdownSeconds 再次发送验证码前的剩余等待秒数，单位为秒。
 * 取值大于 `0` 时发送按钮保持禁用；`0` 表示允许重新发送。
 * 当前成功发送后从 60 开始递减，不允许出现负数。
 *
 * @property errorMessage 等待页面展示的一次性中文错误提示。
 * `null` 表示当前没有待展示错误；非空时由页面通过 Snackbar 展示，
 * 展示后必须调用清理方法恢复为 `null`，避免重组时重复提示。
 *
 * @property loginSuccess 本次登录是否已经成功完成。
 * `true` 时页面执行一次性导航并清空登录返回栈；
 * `false` 表示仍停留在登录流程。该值不是持久会话状态来源。
 *
 * @property user 登录成功后服务端返回并映射得到的当前用户信息。
 * 登录前以及登录失败时为 `null`；当 [loginSuccess] 为 `true` 时原则上应非空。
 * 持久会话恢复不得依赖此字段，应由认证仓库重新读取会话状态。
 */
...
对于字段较多的类型，可以按业务区域分组，但仍然必须逐字段说明。
不得使用“其余字段含义见名称”“字段含义自解释”等方式跳过属性文档。
