...
根据验收标准，
“未验证项不得标记通过”，因此 AC-11
保持待完成。2026-07-02 复核当前 `HEAD=11652fa627c9fa1d715190db75383b7018cd7bd7`：
Android CI 远端 run `28512594215` 已 completed/success，并已刷新到
`docs/migration/evidence/github-actions-android.json`；Android 登录态 Tab 网络观察与退出登录网络观察
仍满足结构化证据要求；macOS/iOS 证据已按当前 HEAD 留存 `overallResult: skipped` 风险记录。
当前仍缺少同一 HEAD 的 iOS CI completed/success 证据，且 skipped 不能替代 iOS runtime pass，
所以 L1 仍不得封板。
...
- `checkL1CiWorkflows` 现在同时检查：
  ...
  - PR 模板要求补丁说明变更目标、影响平台、验证、Android/iOS CI 状态、L1 封板证据状态、
    依赖大版本升级人工回归、剩余风险和回滚方案；真正的 Branch Protection required checks
    仍需 GitHub 远端设置生效。
...
- `checkLongTermGovernance` 已由 `runninghub.long-term-governance` build-logic 插件注册：
  - 校验 `ARCHITECTURE.md`、`DEVELOPMENT.md`、ADR、迁移归档边界和 `docs/governance` 文档入口存在且保留关键治理主题。
  - 将复核建议收敛为 P1-P10，覆盖上下文入口、build-logic 收口、迁移归档、`composeApp` 单体控制、
    Feature Presentation 阈值、日志脱敏、安全凭据存储、API/SQLDelight 契约测试、Compose Resources、性能基线和发布自动化。
  - 使用 `docs/governance/composeapp-file-size-allowlist.txt` 登记既有超大 commonMain Kotlin 文件，
    后续新增超过 800 行的 `composeApp` 文件会失败，避免 UI 单体继续扩大。
  - 使用 `docs/governance/feature-presentation-thresholds.txt` 登记尚无独立 Presentation 模块的
    大体量 UI Feature；后续超过登记行数会失败，倒逼拆分模块或更新治理说明。
  - 扫描生产日志调用行，阻止 Token、Cookie、Authorization、API Key、password、密码、URL
    或媒体文件名等敏感语义进入 `Log.`、`println(`、`Napier.`、`Logger.` 或项目内自定义
    `debug(` 输出。
  ...
  - 使用 `docs/governance/performance-baseline-targets.txt` 固化启动、首屏、内存、图片、视频和长轮询
    指标的证据产物与责任域，并校验证据文件和 Markdown anchor 可追溯。
  - 使用 `docs/governance/release-readiness-checklist.md` 固化 Android Release、iOS TestFlight、
    生产签名/商店上传、App Store 隐私表单人工确认和依赖大版本升级人工回归停止条件，
    并校验 Android release 构建继续启用 R8/minify、资源压缩、ProGuard 规则和
    `verifyL1Android` 的 `:composeApp:assembleRelease` 聚合。
  - 校验 `core:network` 认证头主机判断保留 `ApiEnvironment.trustedAuthHosts` 精确白名单，
    阻止 `contains("runninghub.cn")` 或后缀匹配重新把相似域名判定为站内请求。
  - 校验 Android Keystore、iOS Keychain、`MigratingCredentialStore` 和双端 runtime module
    凭据绑定，防止 `CredentialStore` 退回普通 Preferences；旧凭据只能按字段懒迁移，不能误删
    余额缓存或快捷创作草稿。
  - 校验 iOS PermissionController、PermissionDataStore、MediaResolver 和 Info.plist 保留真实
    PhotoKit 授权、系统选择器、设置页跳转、权限说明和安全作用域 URL 读取，防止 P0-3
    退回固定授权或空函数；真实上传回归仍需 Simulator/真机证据。
  - 校验 `iosApp/iosApp/PrivacyInfo.xcprivacy` 已加入 Xcode 资源，并保留 Required Reason API
    类目、发布清单入口和 App Store 隐私表单人工确认说明；真实 App Store Connect 填报仍由
    发布前人工完成。
  - 校验短信图形验证码的 TAC HTML、Android WebView、iOS WKWebView、脚本消息、scheme 兜底、
    handler 清理和失败重试契约，防止 P0-4 退回不可观察或不可清理的 Web 容器实现；真实
    TAC JS/CSS 加载和双端交互仍需运行证据。
...
./gradlew.bat :feature:discovery:presentation:testDebugUnitTest --tests "com.runninghub.feature.discovery.presentation.DiscoveryStateHolderTest"
...
./gradlew.bat :feature:community:domain:testDebugUnitTest :feature:community:presentation:testDebugUnitTest :composeApp:compileDebugKotlinAndroid :feature:community:domain:compileKotlinIosSimulatorArm64 :feature:community:presentation:compileKotlinIosSimulatorArm64
...
| Gate D 认证与 401 | 通过 | `core:network`、`feature:auth:domain` 单元测试覆盖刷新、重试、并发和 logout 竞态；本轮按复核建议把 `TokenRefresher` 的刷新响应解析从正则改为 kotlinx.serialization DTO，新增 JSON unicode escape token 测试，避免 Authorization 写入未解码的 token 文本；`checkArchitectureBoundaries` 会拒绝在该文件重新引入 Regex/toRegex；本轮继续新增 `ApiEnvironment`，`RunningHubApiEnvironment` 改为平台启动层可配置门面，Android 通过 build type 注入的 `RUNNINGHUB_*` BuildConfig 字段覆盖环境，iOS 通过同名进程环境变量覆盖环境，未配置时回退 production，不再改 Data endpoint；`feature:auth:data` 和 `shared` 遗留 Auth logout 的远端失败降级已从空 `catch` 改为显式 no-log 处理，避免认证请求细节进入日志。 | staging/dev 真实地址尚未登记；后续只需补构建参数或 Xcode scheme。 |
| Gate E Discovery | 通过 | Discovery 依赖窄 `WebAppCatalogRepository`；Domain 已提供 `CatalogQuery`、`CatalogSort`、`CatalogTagRange` 和 `CatalogError`；Data 层集中映射远端协议参数；Presentation 使用 `toCatalogErrorMessage` 映射文案。`DiscoveryStateHolderTest` 已覆盖 index 0 语义、分类/排序/搜索旧响应隔离、分页去重、加载更多失败不推进页码、刷新保留筛选和目录错误文案映射。 | 无。 |
| Gate F QuickCreate | 通过 | 唯一入口、Data 去 `shared`、Coordinator/StateHolder 结构、相关测试和构建通过；本轮已移除 QuickCreate Data 层裸 `println`，并将 `QuickCreateRepositoryImpl` 的 `AuthRepository` 改为必需依赖，测试统一使用 `FakeAuthRepository` 覆盖刷新语义。生成任务状态流本地兜底已迁移为 `QuickCreateTaskIssueCode`，`QuickCreateTaskPollingController` 负责映射中文展示文案，相关 Data 与 Presentation 测试通过；本轮继续收口 QuickCreate 远端 `msg/message/errorMessage`，生成、轮询、旧 OpenAPI 失败和异常兜底只向 Presentation 传播稳定错误码，未知异常统一降级为本地兜底文案。非任务状态 Data 本地兜底已迁移为 `QuickCreateRepositoryIssueCode` 和 `QuickCreateRepositoryException`，历史、项目、灵感、模型、上传和计费预览的最终展示文案由 `QuickCreateErrorMessages` 统一映射，`QuickCreateRepositoryException.message` 固定为 issueCode，不再等于远端摘要；静态搜索确认 Data commonMain 不再保留这些本地兜底异常文案。`QuickCreateHistoryStateHolderTest` 覆盖最近历史和项目任务按 `taskId` 去重、旧最近历史不覆盖选中项目任务；`QuickCreateProjectStateHolderTest` 覆盖项目分页按 `projectId` 去重和删除选中项目后的回调边界。`QuickCreateScreenModelTest` 覆盖上传超时/失败不提交远端任务，并覆盖活跃生成 Job 期间重复点击不会产生第二次远端提交；后续任务状态推进会清理重复点击产生的临时提示；本轮继续覆盖页面 `onDispose` 会取消正在收集的图片生成状态流。旧 OpenAPI queryTask 与 QuickCreation 任务列表收到 CANCELED/CANCELLED 后均映射为取消终态并停止轮询，Presentation 展示“任务已取消”。QuickCreation V2 失败只查询一次任务列表，旧 OpenAPI 失败只查询一次 queryTask；V2 长时间没有终态时在固定次数后发出 `TASK_TIMEOUT` 并结束 Flow。灵感视频模板应用后，编辑状态与计费预览请求共享同一份模型、动态参数和模板媒体。生成提交流程在点击生成时捕获状态快照，等待上传期间继续编辑 prompt 不会污染当前远端提交，上传完成只回填快照素材的远端 URL。计费预览成功后保存请求指纹，正式提交会按最终请求重新计算并匹配；上传完成但未重新计费的请求会以“运行前确认费用”拦截。成功提交进入 Queuing 后会清理本地草稿。composeApp commonMain 与 feature/quickcreate 源码静态搜索无裸 `println`。本轮继续按复核建议隔离旧 Create 页面：旧 `CreateVoyagerScreen` 已变为 internal 且 ERROR 级废弃，架构门禁会阻止其他生产 commonMain 文件重新构造旧入口。登录态 Tab 网络观察已证明切换 History/QuickCreate 等一级 Tab 后稳定窗口内无持续新增网络请求；Android 退出登录后 Login 根页面空闲观察已证明稳定窗口内没有持续后台请求。 | 无。macOS iOS Simulator 运行路径以 `overallResult: skipped` 记录风险，后续具备 macOS 环境后可补齐真实 pass 证据。 |
...
| Gate H Core 与 Shared 收口 | 通过 | 删除空壳 designsystem；`core:model` 的 User、WebApp、Tag、PageData、AppDetail 及嵌套业务模型已补齐字段级中文 KDoc，并通过 Android 与 iOS Simulator Kotlin 编译；`core:common` 新增旧登录协议专用 `md5` 跨平台入口；权限模型、权限状态、权限存储边界、Android Manifest 映射、DataStore 平台工厂、DataStore-backed 余额/QuickCreate 草稿和权限状态存储实现已迁入 `core:storage`；AC-11 复核补丁已把生产 `CredentialStore` 切换到 Android Keystore backed 密文存储与 iOS Keychain，并通过 `MigratingCredentialStore` 懒迁移旧 DataStore 凭据；WebApp 任务提交、输出、历史和任务执行状态模型已迁入 `core:model`；统一生成历史模型、`GenerationHistoryRepository` 和 `WebAppTaskRepository` 已迁入 `feature:task:domain`；Plaza 模型与 `PlazaRepository` 已迁入 `feature:community:domain`，Plaza API/DTO/Repository/DI 和测试已迁入 `feature:community:data`，shared 不再保留 Plaza Data 绑定；WebApp 公开目录、搜索、标签树、用户发布列表和详情 API/DTO/Repository/DI 已迁入 `feature:discovery:data`，shared 不再保留公开目录 endpoint 或 `WebAppCatalogRepository` 绑定；认证、用户资料、会话恢复、个人中心凭据和余额快照实现已迁入 `feature:auth:data`，shared 中旧 RunningHubApi、Auth/User DTO、Auth/User/Session/ProfileCredential/BalanceSnapshot Repository 和对应自测已删除；WebApp Task API/DTO/Repository/DI 和测试已迁入 `feature:task:data`，shared 不再保留 Task endpoint、DTO、Repository 实现或 Data 绑定；Android 生产启动图不再装配 shared，新增 shared baseline 和 ownership 文档。2026-06-21 复核 `composeApp` debugCompileClasspath 无 `shared` 依赖，`composeApp`、`feature` 和 `core` 当前生产模块无实际 `shared` 引用。2026-06-22 中期瘦身继续推进：历史 `androidApp` 跟踪源码已删除；Audio 领域模型和仓库契约已迁入 `feature:audio:domain`，Audio API/DTO/Repository/DI 和测试已迁入 `feature:audio:data`；标准模型目录模型、调用模型和仓库契约已迁入 `feature:model:domain`，标准模型 API/DTO/Repository/DI 和测试已迁入 `feature:model:data`；User/WebApp/Tag/PageData/AppDetail 的 shared typealias 兼容层已删除，shared 不再保留业务 Domain 或业务 Data 实现。 | 无。shared 剩余旧兼容文件已登记归属与删除条件，属于 L1 后继续瘦身项；真机 Keychain/Keystore 升级回归、备份/卸载/系统还原风险检查归 AC-12。 |
...
| Gate J CI | 待 iOS CI 当前 HEAD 证据 | 当前 `HEAD=11652fa627c9fa1d715190db75383b7018cd7bd7` 已推送到 `origin/feature/kmp-refactoring`；Android CI run `28512594215` 已 completed/success，`github-actions-android.json` 已刷新到当前 HEAD；Android 登录态 Tab 网络观察与退出登录网络观察 JSON 仍满足结构化校验字段；macOS/iOS 证据已刷新为当前 HEAD 的 `overallResult: skipped` 风险记录。`iOS CI` workflow 当前为 `workflow_dispatch` 证据入口，需在真实 macOS Simulator 登录、退出和 QuickCreate 冒烟后填写 `simulator_smoke_pass=true` 与 `smoke_notes`，再采集同一 HEAD 的 successful run。 | 当前缺少 `HEAD=11652fa627c9fa1d715190db75383b7018cd7bd7` 的 iOS CI completed/success 证据；`github-actions-ios.json` 仍绑定旧 HEAD `85f134d23ac58768e8a40c3172f3fbb34ca90699`，不能用于当前封板。skipped macOS/iOS 记录只是风险留存，不是 iOS runtime pass。 |

## 下一步

AC-11 的本地修复、Windows 可执行门禁、Android CI 当前 HEAD 证据和 Android 运行观察证据已补齐，
但 L1 仍未封板。下一步只剩 iOS/macOS 外部证据补采：

1. 在 macOS Simulator 上真实覆盖登录、退出和 QuickCreate 操作路径后，手动触发 `iOS CI`
   的 `workflow_dispatch` 证据模式，填写 `simulator_smoke_pass=true` 和非空 `smoke_notes`。
2. iOS workflow successful 后，重新运行
   `docs/migration/collect-github-actions-evidence.ps1 -HeadSha 11652fa627c9fa1d715190db75383b7018cd7bd7`
   以刷新 `github-actions-ios.json`，并用
   `docs/migration/download-ios-macos-evidence.ps1 -RunId <run id>` 下载
   `ios-macos-link-and-simulator.md` pass artifact。
3. 最终只在外部证据文件发生变化且没有其他工作区差异时运行
   `.\gradlew.bat --console=plain checkL1SealEvidence`；通过前不得把 AC-11 或 L1 标为封板。
