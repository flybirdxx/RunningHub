...
QuickCreate 图片质量“低/中/高”已从 `QuickCreateEditorUiModels.ImageQuality` 的中文 `displayName` 改为稳定 `ImageQualityLabel` 语义；`QuickCreateClassicComposerContent` 通过 composeApp Compose Resources 映射质量 chip 文案，旧接口 `apiValue` 保持不变，本地价格估算已移除，最终计费以后端 fee preview 或服务端目录价为准，`QuickCreateEditorUiModels` 硬编码 UI 文案基线从 11 下调到 8。
...
Compose Resources 映射视频参数 chip 文案，旧接口 `apiValue`、`seconds` 保持不变，本地价格估算已移除，`QuickCreateEditorUiModels`
