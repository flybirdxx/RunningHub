...
QuickCreate 任务轮询失败和错误状态的任务错误码文案已从 `QuickCreateTaskPollingController` 的私有中文映射收口到 `QuickCreateErrorMessages.toQuickCreateTaskIssueDisplayMessage`；控制器只负责任务状态转换，未知远端摘要仍统一降级为通用生成失败文案，避免 `msg/message` 直接进入页面，`QuickCreateTaskPollingController` 硬编码 UI 文案基线移除。
...
commonMain Compose Resources，硬编码 UI 文案基线从 7 下调为 0；本轮 AppDetail 详情加载失败、任务提交失败、任务失败和任务超时错误文案也从 Presentation 中文 String 改为稳定 `AppDetailErrorText`，由 AppDetailScreen 通过 Compose Resources 映射最终文案，Detail Presentation 硬编码错误文案基线移除；服务端字段识别关键字、输入节点标题/选项、应用标题、作者和任务输出文件名仍保持运行时或 Presentation 映射数据。
