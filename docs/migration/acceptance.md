...
Gate H/P7 本轮继续瘦身 `shared` 直接依赖：`sharedModule` 组合入口已删除，当前 `shared`
源码只保留平台信息和 MD5 兼容工具；仅由该组合入口需要的 `core:network`、`core:storage`、
`feature:auth:domain`、Ktor、Koin、serialization 和 DataStore 依赖已从 `shared/build.gradle.kts`
移除。`:shared` 模块本身暂时保留，用于承接仍显式编译 shared framework 的验证入口。
