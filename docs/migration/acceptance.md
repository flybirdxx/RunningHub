...
`verifyL1Ios` 不再显式编译 shared framework。架构门禁从 shared allowlist/baseline
增长控制改为直接拒绝重新 include、重新依赖或重新跟踪 `shared/` 文件。
