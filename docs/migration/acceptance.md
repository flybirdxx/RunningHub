...
- 当前任务：中期架构整理，按复核长期建议逐步拆分 Presentation/Data/Core 边界；用户可见提示/错误文案先按 `docs/governance/ui-copy-message-inventory.md` 全量盘点和分类后批量收口；暂不补上线 CI、Release 或 L1 封板证据。
...
| Gate F QuickCreate | 完成 | Presentation 已拆出多个组件；唯一入口、Data 去 shared、Data 层无裸 `println`、`AuthRepository` 非可空依赖已完成；生成任务状态流和非任务状态 Data 本地兜底均已改为 Domain 稳定错误码，并由 Presentation 映射展示文案；历史/项目分页去重和项目任务状态覆盖已有 StateHolder 测试；上传失败/超时拦截、单一生成任务，成功、失败、取消、超时轮询终态、页面 `onDispose` 取消活跃图片生成状态流和活跃媒体上传、模板应用一致性、生成提交快照漂移，以及计费预览与正式提交请求指纹一致性已有测试覆盖；`QuickCreateMediaUploadCoordinator` 已迁入 `feature:quickcreate:presentation/upload`，composeApp 只保留平台 `MediaResolver` 适配与页面级编排；composeApp commonMain 与 feature/quickcreate 源码静态搜索无裸 `println`。P8 UI 文案资源化本轮继续收缩 QuickCreate 叶子 UI：结果页标题/清空结果入口、灵感页空状态/加载更多入口已迁入 commonMain Compose Resources，对应硬编码文案基线从 1 下调到 0；模型选择面板标题、加载态、空态和关闭按钮无障碍描述也已迁入 commonMain Compose Resources，`QuickCreateModelSelectorContent` 基线从 1 下调到 0；生成按钮计费文案从 `QuickCreateBillingUiText` 的中文 String 改为稳定 `QuickCreateSendButtonLabel`，compact/classic 两个按钮通过 composeApp Compose Resources 映射最终文案，该文件硬编码 UI 文案基线移除；任务状态区域缺省文案和提交/排队/运行进度/上传等待/成功/取消固定文案均改为稳定 `QuickCreateTaskStatusText`，`QuickCreateResultContent` 通过 composeApp Compose Resources 映射最终文案；动态上传字段的格式、数量和大小提示从 `QuickCreationServiceFieldUiModel` 的中文拼接改为稳定 `QuickCreationServiceUploadHint`，参数面板通过 composeApp Compose Resources 映射最终提示，该文件硬编码 UI 文案基线移除；项目列表任务数、置顶无障碍、置顶状态、删除确认和详情行标签/值从 `QuickCreateProjectUiModel` 的中文 String 改为稳定语义模型，`QuickCreateProjectContent` 通过 composeApp Compose Resources 映射最终文案，该文件硬编码 UI 文案基线移除；QuickCreate 错误码端口已改为 `QuickCreatePresentationError`，历史、项目、灵感、计费预览和任务轮询错误由 `quick_create_error_*` Compose Resources 映射最终文案；模型名称、分组、副标题、模板标题、标签和服务端字段标题/描述/占位仍由 Presentation 状态/文案映射提供，后续应按独立文案端口切片处理。登录态 Tab 网络观察已证明切换 History/QuickCreate 等一级 Tab 后稳定窗口内无持续新增网络请求；Android 退出登录后 Login 根页面空闲观察已证明稳定窗口内无新增请求和 in-flight 请求。macOS iOS Simulator 路径按用户确认以 skipped 风险证据留存。 |
| Gate F2 Community Presentation | 进行中 | 2026-06-22 中期瘦身继续推进：新增 `feature:community:presentation`，把 Plaza 状态机、筛选、分页、短片加载和 fallback 内容逻辑迁出 `composeApp`；`PlazaScreenModel` 现在只负责 Voyager 生命周期适配，测试迁入 `PlazaStateHolderTest`。本轮继续把 Plaza 错误状态从 `Throwable.message`/字符串收口为 `PlazaPresentationError`，可见文案由 composeApp Compose Resources 映射，`PlazaStateHolderTest` 覆盖灵感和短片失败不会把异常消息写入 UI 状态。Plaza 本地 fallback 内容目录已从最终展示文本改为 `PlazaFallbackTagLabel`/`PlazaFallbackCardText` 稳定语义，`PlazaScreen` 通过 Compose Resources 映射内置标签、卡片简介、作者和媒体类型，服务端返回的广场内容仍作为运行时数据展示；资源映射导入导致 `composeApp/.../feature/plaza` 壳层目录按治理插件口径达到 860 行，已在 `feature-presentation-thresholds.txt` 登记中期基线，状态机职责未回流。Community 工具页的 `CommunityTool`、`CommunityUiState` 和静态工具目录已迁入同一 Presentation 模块；`CommunityScreenModel` 现在只负责 Voyager/Koin 适配，工具点击后的真实导航仍保留在应用壳。P8 UI 文案资源化已继续收缩 Community 页头和工具目录：页头标题/副标题、6 个工具标题和说明均迁入 commonMain Compose Resources；`CommunityTool` 只保留稳定 `titleKey`/`descriptionKey`，`CommunityScreen` 负责资源映射，Community Presentation 不再登记硬编码 UI 文案基线。本轮继续清理 Plaza UI 壳体本地颜色别名，状态机职责未回流。 |
| Gate F3 Task History Presentation | 进行中 | 2026-06-22 中期框架整理继续推进：新增 `feature:task:presentation`，把 History 的状态、列表筛选、详情加载、参数复用、取消任务和普通历史轮询迁出 `composeApp`；`TaskHistoryScreenModel` 现在只负责 Voyager 生命周期适配，测试迁入 `TaskHistoryStateHolderTest`。P8 UI 文案资源化继续收缩 History 页面：页面标题、搜索/刷新无障碍描述、项目条、置顶项目、筛选 Tab、提示条、空态、详情面板标题、日期分组、任务 ID 格式、操作按钮、状态标签、参考 fallback 任务标题和错误兜底均迁入 commonMain Compose Resources，`TaskHistoryScreen` 硬编码 UI 文案基线从 14 下调到 0；本轮继续把 `TaskHistoryUiState.actionMessage/error` 从中文 `String?` 收口为 `TaskHistoryActionMessage?` / `TaskHistoryPresentationError?`，`TaskHistoryFilter.label` 删除，History 操作提示、详情失败、取消失败、历史加载失败和登录同步提示由 `TaskHistoryScreen` 通过 Compose Resources 映射，UI 不再通过 `TOKEN/401` 字符串内容判断鉴权错误。`TaskHistoryStateHolderTest` 覆盖无复用参数、无重试参数、详情失败和鉴权失败分支；TaskHistoryStateHolder 的硬编码 UI 文案基线移除。任务标题、输出 URL、任务 ID、复用参数键值和服务端运行时数据仍保持运行时或 Presentation 数据。History UI 壳仍留在 composeApp，体量基线只因资源化导入增长而临时更新，状态机职责未回流。 |
...
| Gate F5 Discovery Presentation | 进行中 | 2026-06-22 中期框架整理继续推进：新增 `feature:discovery:presentation`，把 Discovery 的 UiState、排序展示名、目录错误文案、分类/排序/分页/搜索状态机和旧响应隔离迁出 `composeApp`；`DiscoveryScreenModel` 现在只负责 Voyager 生命周期适配，测试迁入 `DiscoveryStateHolderTest`。本轮继续把独立 Search 页的 `SearchUiState`、热门标签加载、输入防抖搜索、分页、错误文案映射和旧响应隔离迁入同一 Presentation 模块；`SearchScreenModel` 现在只负责 Voyager 生命周期适配。本轮继续把 Discovery/Search 的目录错误状态从中文 `String` 收口为 `CatalogPresentationError`，可见错误文案由 composeApp Compose Resources 映射，Discovery Presentation 不再登记目录错误硬编码 UI 文案基线。本轮按中期开发范围验证新模块单测、Android 编译、composeApp Android 编译和 iOS Simulator Kotlin 编译，不补上线 CI/Release 封板证据。 |
| Gate F6 Auth Login Presentation | 进行中 | 2026-06-22 中期框架整理继续推进：把 Login 的 `LoginUiState`、短信验证码发送、图形验证码 token 重试、短信/密码登录、倒计时和错误文案映射迁入 `feature:auth:presentation`；`LoginScreenModel` 现在只负责 Voyager 生命周期适配，`SmsCaptchaDialog` expect/actual、TAC HTML 和回调解析继续保留在 `composeApp` 平台边界。本轮继续把 Login 本地校验、短信、密码、图形验证码和网络失败提示从中文 `String` 收口为稳定 `LoginErrorText`，可见 Snackbar 文案由 composeApp Compose Resources 映射，未知 `Throwable.message` 不再透传到页面状态，Auth Login Presentation 不再登记硬编码 UI 文案基线。本轮继续收口短信图形验证码 Web 容器文案：弹窗标题、关闭按钮、TAC 标题、加载/失败/重试/成功提示由 `SmsCaptchaCopy` 从 Compose Resources 注入，HTML 只负责转义和 TAC 配置，不再硬编码最终中文 UI 文案。本轮继续修正验证码平台边界：commonMain HTML 保持 `/tac` 与 `/uc` 相对路径，Android WebView 和 iOS WKWebView 的 baseURL 由平台环境门面提供，并由 `SmsCaptchaEnvironmentTest` 与 `SmsCaptchaHtmlTest` 锁定。本轮按中期开发范围验证 Auth Presentation 单测、Android/iOS Kotlin 编译、验证码保留测试、Android debug 构建、架构边界和长期治理门禁，不补上线 CI/Release 封板证据。 |
| Gate F7 AppDetail Presentation | 进行中 | 2026-06-22 中期框架整理继续推进：新增 `feature:detail:presentation`，把 AppDetail 的 `AppDetailUiState`、媒体选择/上传状态、详情加载、任务提交、输出轮询和失败文案映射迁出 `composeApp`；`AppDetailScreenModel` 现在只负责 Voyager 生命周期、平台媒体读取适配和 UI 事件转发，AppDetail 页面继续保留权限、媒体选择器和渲染逻辑。本轮继续把 AppDetail 的详情加载失败、任务提交失败、任务失败和轮询超时状态从中文 `String` 收口为稳定 `AppDetailErrorText`，可见错误文案由 composeApp Compose Resources 映射，Detail Presentation 不再登记硬编码 UI 文案基线；输入节点字段类型分派、服务端选项消费、媒体类型启发式、默认值优先级和连续图片上传分组已迁入 `AppDetailInputPresentation` 结构化模型，composeApp 只负责资源映射、权限、平台媒体选择和控件渲染。本轮新增 `AppDetailStateHolderTest` 与 `AppDetailInputPresentationTest` 并保留 composeApp 适配测试，按开发中期范围验证新模块单测、composeApp 适配单测、Android Kotlin 编译和 Detail Presentation iOS Simulator Kotlin 编译；不补上线 CI/Release/L1 封板证据。 |
...
Gate H/P7 本轮完成 `:shared` 模块整体退役：最后的 Platform/MD5 兼容文件和
`shared/build.gradle.kts` 已删除，`settings.gradle.kts` 不再 include `:shared`，
`verifyL1Ios` 不再显式编译 shared framework。架构门禁从旧增长控制文件
改为直接拒绝重新 include、重新依赖或重新跟踪 `shared/` 文件。
...
P2 根构建脚本基线继续收紧：`build.gradle.kts` 当前实际行数为 1789，`build-script-baseline`
已从 1800 下调到 1789，防止新验收逻辑继续堆回根脚本。
...
commonMain Compose Resources，硬编码 UI 文案基线从 5 下调为 0；搜索词、热门标签名、应用标题和作者仍保持运行时数据。
LoginScreen 的品牌展示、手机号/验证码/密码标签、区号、发送验证码状态、登录按钮、登录方式切换和协议提示也已迁入
commonMain Compose Resources，硬编码 UI 文案基线从 8 下调为 0；本轮登录本地校验、短信、密码、图形验证码和网络错误也从 Auth Presentation 中文 String 改为稳定 `LoginErrorText`，由 LoginScreen 通过 Compose Resources 映射最终 Snackbar 文案，未知 `Throwable.message` 不再进入页面状态。
...
QuickCreate 生成提交和媒体上传等待的运行时状态文案已从裸 `String` 改为 `QuickCreateTaskStatusText` 稳定语义，并由 `QuickCreateResultContent` 通过 composeApp Compose Resources 映射最终文案；`QuickCreateMediaUploadCoordinator` 已整体迁入 `feature:quickcreate:presentation/upload`，`QuickCreateGenerationInteractor` 已迁入 `feature:quickcreate:presentation/generation`，`QuickCreateCoordinator` 已迁入 `feature:quickcreate:presentation/coordinator`；composeApp 只保留 Voyager ScreenModel 门面，并由 `AppModule` 组合根把平台 `MediaResolver` 适配为 `QuickCreateMediaResolver` 后注入 ScreenModel 和 feature presentation。生成请求构建器的 Prompt 阻塞原因已从中文 `String` 改为稳定 `QuickCreateGenerationBlockReason`，本轮继续把 `QuickCreateRuntimeUiText` 从中文文案端口改为稳定语义类型，`QuickCreateUiState.error`、`feePreviewError` 和 `MediaReference.errorMessage` 改为 `QuickCreateUiMessage?`；生成阻塞、计费待确认、重复生成、Prompt 校验和素材上传失败/阻塞/超时提示由 `QuickCreateUiMessageText` 通过 composeApp Compose Resources 映射，`QuickCreateRuntimeUiText` 的硬编码 UI 文案基线移除。
QuickCreate 服务字段校验已从 `QuickCreationServiceSchema` 拼接中文 `String` 改为
`QuickCreationServiceValidationIssue` 稳定语义；`QuickCreateGenerationRequestFactory`
包装为 `QuickCreateGenerationBlockReason.ServiceValidation`，`QuickCreateGenerationInteractor`
映射到 `QuickCreateRuntimeUiText`，composeApp 通过 `quick_create_runtime_service_*`
资源格式串生成最终文案。服务端字段标题、字段说明和选项标签仍作为运行时数据展示，
不作为客户端静态 UI 文案资源化。
QuickCreate 计费预览拦截和失败 fallback 已从最终中文文案改为 `QuickCreatePresentationError` 稳定语义；`QuickCreateFeePreviewInteractor` 只引用统一错误语义端口，并保留原计费防抖、旧响应隔离和请求指纹行为。
QuickCreate 灵感标签、模板分页和模板详情失败已从 `QuickCreateInspirationStateHolder` 的本地 fallback 文案收口为 `QuickCreatePresentationError`；StateHolder 只写入 `QuickCreateUiMessage.PresentationErrorText`，对应测试覆盖未知异常不会透传到页面状态。
QuickCreate 项目列表、置顶、创建、重命名、删除和详情加载失败已从 `QuickCreateProjectStateHolder` 的中文 fallback 收口为 `QuickCreatePresentationError`；项目错误码映射和 StateHolder 共用同一批稳定错误语义，对应硬编码 UI 文案基线移除。
QuickCreate 最近历史分页、取消任务、历史详情、项目任务列表和轮询刷新失败已从 `QuickCreateHistoryStateHolder` 的中文 fallback 收口为 `QuickCreatePresentationError`；历史错误码映射同步复用稳定错误语义，轮询刷新继续作为 Presentation 场景错误，不扩 Domain 错误码。
QuickCreate 任务轮询失败和错误状态已从 `QuickCreateTaskPollingController` 的私有中文映射收口为 `QuickCreatePresentationError`；任务失败状态区使用 `QuickCreateTaskStatusText.Error(error)`，composeApp 通过 `quick_create_error_*` Compose Resources 映射最终文案，未知远端摘要仍降级为通用生成失败语义，避免 `msg/message` 直接进入页面。
QuickCreate 图片质量“低/中/高”已从 `QuickCreateEditorUiModels.ImageQuality` 的中文 `displayName` 改为稳定 `ImageQualityLabel` 语义；`QuickCreateClassicComposerContent` 通过 composeApp Compose Resources 映射质量 chip 文案，旧接口 `apiValue` 和本地价格估算不变，`QuickCreateEditorUiModels` 硬编码 UI 文案基线从 11 下调到 8。
QuickCreate 视频分辨率“原生1080p”和视频时长“5秒/10秒”已从 `VideoResolution`/`VideoDuration` 的中文 `displayName`
改为稳定 `VideoResolutionLabel`/`VideoDurationLabel` 语义；`QuickCreateClassicComposerContent` 通过 composeApp
Compose Resources 映射视频参数 chip 文案，旧接口 `apiValue`、`seconds` 和本地价格估算不变，`QuickCreateEditorUiModels`
硬编码 UI 文案基线从 8 下调到 5。
QuickCreate 本地兼容图片/视频模型名已从 `ImageModel`/`VideoModel` 的中文 `displayName` 改为
`ImageModelLabel`/`VideoModelLabel` 稳定语义；`QuickCreateCompactComposerContent` 与
`QuickCreateParamsSheetContent` 通过 composeApp Compose Resources 映射全能图片、万相和可灵等固定中文模型名，
英文第三方商品名继续作为运行时文本展示；`QuickCreateEditorUiModels` 剩余 5 个硬编码 UI 文案基线移除，
当前 `ui-copy-hardcoded-baseline.txt` 无正数文件。
composeApp 静态 UI 文案批次继续收口：`AppSearchBar` 的默认占位、搜索和清除无障碍描述，
QuickCreate 顶栏返回/客服/静音无障碍描述，媒体卡素材类型、上传状态、失败 fallback 和移除无障碍描述，
以及上传工具条/字段上传入口的素材标签、数量格式、上传计数和默认提示均迁入 Compose Resources；
目标文件静态扫描确认无中文字符串字面量残留。
...
QuickCreate 服务模型标题、默认分组和参数数量副标题已从 `QuickCreateServiceModelUiModel` 的中文 `String` 改为稳定的 `QuickCreateServiceModelDisplayName`、`QuickCreateServiceModelGroupTitle` 和 `QuickCreateServiceModelSubtitle`；`QuickCreateModelSelectorContent` 与 `QuickCreateClassicComposerContent` 通过 composeApp Compose Resources 映射未命名模型、其他模型、参数数量和副标题分隔符，服务端模型名/分组名仍作为运行时数据展示，compact 名称清洗只保留运行时服务端文本处理，该文件硬编码 UI 文案基线移除。
...
commonMain Compose Resources，硬编码 UI 文案基线从 7 下调为 0；本轮 AppDetail 详情加载失败、任务提交失败、任务失败和任务超时错误文案也从 Presentation 中文 String 改为稳定 `AppDetailErrorText`，由 AppDetailScreen 通过 Compose Resources 映射最终文案，Detail Presentation 硬编码错误文案基线移除；SWITCH 字段缺少服务端 options 时不再展示本地假选项“选项A/选项B”，改为复用已资源化的布尔开关控件；输入节点字段类型分派、服务端选项消费、媒体类型启发式、默认值优先级和连续图片上传分组已迁入 `feature:detail:presentation` 的结构化 UI model，AppDetailScreen 只保留控件渲染、权限和平台媒体选择器映射；输入节点标题/选项、应用标题、作者和任务输出文件名仍保持运行时或 Presentation 映射数据。
DiscoveryScreen 的搜索/关闭/排序无障碍描述、主列表空态/到底提示、内联搜索提示/空态/标题、首页模型 Banner 静态文案、全部分类、全部应用标题、作者兜底和卡片统计无障碍标签也已迁入
commonMain Compose Resources，硬编码 UI 文案基线从 11 下调为 0；目录分类名、应用标题、作者、Banner tile 标题与媒体 URL 和搜索词仍保持运行时数据。
Discovery 排序下拉的“推荐/口碑/最热/最新”固定标签已从 `feature:discovery:presentation`
迁入 composeApp commonMain Compose Resources；`DiscoveryStateHolder` 不再持有排序展示文案。目录错误状态已从中文 `String`
收口为 `CatalogPresentationError`，Discovery/Search 可见错误文案由 composeApp Compose Resources 映射。
...
| AC-11 | 执行双端回归并封板 L1 | 中期暂停封板补证 | 本地回归、Android lint/debug 构建、iOS Simulator Kotlin 编译、架构门禁、登录态 Tab 网络观察、Android 退出登录后 Login 根页面空闲网络观察，以及多项 ScreenModel/StateHolder/Repository 测试已通过；本轮复核后继续修复认证 Host 精确白名单、iOS 权限/媒体选择占位、Android/iOS 图形验证码回调一致性和相关测试，补强 TAC 脚本加载失败/超时的可重试降级状态，把短信图形验证码 baseURL 改为跟随 `RunningHubApiEnvironment.WEB_BASE_URL`，把 Token 刷新响应从正则解析改为 kotlinx.serialization DTO 解码，把旧 Create 兼容实现从 composeApp 删除并纳入 `checkArchitectureBoundaries` 防回归检查，并把生产 `CredentialStore` 切换到 Android Keystore / iOS Keychain backed 安全存储，旧 DataStore 凭据通过迁移包装器逐项迁移清理。当前未完成项是封板外部证据：按用户要求暂不补上线级 CI、Release 或 L1 封板证据，未来恢复封板时需对目标提交重新采集远端 Android/iOS CI 与 macOS iOS link/Simulator 证据。 |
