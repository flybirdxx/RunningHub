...
最终 L1 证据必须同时包含 `linkResult: pass`、`xcodebuildResult: pass` 和
`simulatorSmokeResult: pass`；缺少 macOS link/xcodebuild/Simulator Markdown 证据时，
L1 仍不能封板。
...
`checkL1SealEvidence` 和 `checkMigrationScripts`，但不能替代 macOS iOS
link/xcodebuild/Simulator Markdown 证据。
