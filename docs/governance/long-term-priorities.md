...
| P5 | Feature Presentation 阈值 | `checkLongTermGovernance` 按 `feature-presentation-thresholds.txt` 约束无同名 Presentation 模块的大体量 UI Feature，并校验真实 ownerPresentation 模块存在 | 超过登记基线和小幅缓冲后继续增长会失败；缓冲内出现可变状态流、协程编排、轮询或组合 Flow 等状态机回流信号仍会失败；baseline 缺少 `feature:<owner>:presentation` 或 owner 模块不存在会失败；除非拆分同名 Presentation 模块或更新治理说明 | 具体模块拆分需产品迭代配合 |
