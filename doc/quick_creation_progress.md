...
| Phase 5 | 历史/项目 | 部分完成 | 50% | `list/detail` DTO/API 已可解析，domain repository 已公开历史分页和按 `outputId` 获取详情；创作页中间区域已展示最近创作并可打开详情弹窗，分页刷新、取消任务和项目管理交互尚未接入。 |
...
| 测试 | `composeApp/src/commonTest/kotlin/com/runninghub/app/ui/feature/quickcreate/QuickCreateScreenModelTest.kt` | 覆盖 ScreenModel 初始化加载 quick-creation 历史、选择 output 后按 `outputId` 加载详情并写入 UI state。 |
...
| UI | `QuickCreateUiState.kt` / `QuickCreateScreenModel.kt` / `QuickCreateScreen.kt` | 新增历史加载状态、历史列表 state 和详情 state；创作页在无当前结果/任务时展示最近创作，支持图片/视频预览、状态、分类和扣费金额摘要；点击历史项按 `outputId` 加载详情弹窗；生成成功后刷新历史。 |
...
| 历史能力仍不完整 | 已能在创作页展示最近创作并打开详情弹窗，但分页加载、运行中任务定时刷新和取消任务仍未接入 | 下一步补分页/刷新策略和取消任务接口。 |
