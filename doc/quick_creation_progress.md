...
| Phase 5 | 历史/项目 | 部分完成 | 90% | `list/detail/cancel/project/list/project/tasks` DTO/API 已可解析，domain repository 已公开历史分页、按 `outputId` 获取详情、按 `taskId` 取消任务、项目列表分页和项目任务分页；创作页中间区域已展示最近创作，可打开详情弹窗，已接入加载更多分页，会对非终态历史任务定时刷新，并可取消非终态任务；历史区顶部已展示项目筛选条，点击项目会加载项目内任务并可切回最近创作；其它项目管理操作尚未接入。 |
...
| 测试 | `composeApp/src/commonTest/kotlin/com/runninghub/app/ui/feature/quickcreate/QuickCreateScreenModelTest.kt` | 覆盖 ScreenModel 初始化加载 quick-creation 历史和项目列表、选择项目后加载项目内任务、清除项目筛选后回到最近创作、加载更多追加下一页、非终态历史任务定时刷新到终态、取消历史任务后刷新列表、选择 output 后按 `outputId` 加载详情并写入 UI state。 |
...
| 历史能力仍不完整 | 已能在创作页展示最近创作、项目横向筛选、项目内任务、加载更多历史、定时刷新非终态任务、取消非终态任务并打开详情弹窗；但项目创建/重命名/删除/置顶/详情等管理动作仍未接入，且非空项目真实响应仍需抓包校正 | 下一步补项目创建/重命名/删除/置顶/详情交互，并在有非空项目时校正 `project/tasks` 请求/响应结构。 |
