...
| Phase 5 | 历史/项目 | 部分完成 | 60% | `list/detail` DTO/API 已可解析，domain repository 已公开历史分页和按 `outputId` 获取详情；创作页中间区域已展示最近创作，可打开详情弹窗，并已接入加载更多分页；运行中刷新、取消任务和项目管理交互尚未接入。 |
...
| 测试 | `composeApp/src/commonTest/kotlin/com/runninghub/app/ui/feature/quickcreate/QuickCreateScreenModelTest.kt` | 覆盖 ScreenModel 初始化加载 quick-creation 历史、加载更多追加下一页、选择 output 后按 `outputId` 加载详情并写入 UI state。 |
...
| UI | `QuickCreateUiState.kt` / `QuickCreateScreenModel.kt` / `QuickCreateScreen.kt` | 新增历史加载状态、分页 state、历史列表 state 和详情 state；创作页在无当前结果/任务时展示最近创作，支持图片/视频预览、状态、分类和扣费金额摘要；历史列表可加载更多并去重追加；点击历史项按 `outputId` 加载详情弹窗；生成成功后刷新历史。 |
...
| 历史能力仍不完整 | 已能在创作页展示最近创作、加载更多历史并打开详情弹窗，但运行中任务定时刷新和取消任务仍未接入 | 下一步补运行中刷新策略和取消任务接口。 |
