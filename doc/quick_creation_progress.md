...
| Phase 5 | 历史/项目 | 部分完成 | 68% | `list/detail` DTO/API 已可解析，domain repository 已公开历史分页和按 `outputId` 获取详情；创作页中间区域已展示最近创作，可打开详情弹窗，已接入加载更多分页，并会对非终态历史任务定时刷新；取消任务和项目管理交互尚未接入。 |
...
| 测试 | `shared/src/commonTest/kotlin/com/runninghub/shared/data/repository/QuickCreateRepositoryImplHistoryTest.kt` | 覆盖 `/task/quick-creation/list` 历史分页映射、扣费字段、`apiRequestParams` 解析、输出尺寸解析，以及 `/task/quick-creation/detail` 按 `outputId` 获取详情。 |
| 测试 | `composeApp/src/commonTest/kotlin/com/runninghub/app/ui/feature/quickcreate/QuickCreateScreenModelTest.kt` | 覆盖 ScreenModel 初始化加载 quick-creation 历史、加载更多追加下一页、非终态历史任务定时刷新到终态、选择 output 后按 `outputId` 加载详情并写入 UI state。 |
...
| 数据层 | `QuickCreateRepository.kt` / `QuickCreateRepositoryImpl.kt` | 新增 `QuickCreationHistoryPage/Item/Output` domain 模型和 `listQuickCreationHistory/getQuickCreationHistoryDetail` 方法，后续 UI 可直接消费 quick-creation v2 历史数据。 |
...
| UI | `QuickCreateUiState.kt` / `QuickCreateScreenModel.kt` / `QuickCreateScreen.kt` | 新增历史加载状态、分页 state、历史列表 state 和详情 state；创作页在无当前结果/任务时展示最近创作，支持图片/视频预览、状态、分类和扣费金额摘要；历史列表可加载更多并去重追加；非终态历史任务会每 5 秒刷新当前已加载范围；点击历史项按 `outputId` 加载详情弹窗；生成成功后刷新历史。 |
...
| 历史能力仍不完整 | 已能在创作页展示最近创作、加载更多历史、定时刷新非终态任务并打开详情弹窗，但取消任务仍未接入 | 下一步先确认 Web 取消任务真实接口，再补 repository 和 UI 入口。 |
