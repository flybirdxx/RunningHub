...
| Phase 3A | 服务端模型字段接入 | 部分完成 | 45% | 已接 `/api/qc/v2/models` DTO、mapper、repository 和 ScreenModel 状态；服务端字段/options/defaultValue 已解析，动态调参表单仍保留本地枚举 fallback。 |
...
| 测试 | `composeApp/src/commonTest/kotlin/com/runninghub/app/ui/feature/quickcreate/QuickCreateScreenModelTest.kt` | 增加创作/灵感模式切换测试、服务端模型加载默认选中测试，以及图片生成请求携带服务端模型 ID 的回归测试。 |
...
| UI | `QuickCreateScreen.kt` / `TuneBottomSheet.kt` | 底部输入区展示当前服务端模型摘要，Tune 高级页展示并切换服务端图片模型，保留本地模型作为兼容 fallback。 |
...
| 服务端字段尚未完全驱动调参表单 | 已能解析和选中模型，但分辨率、比例、质量等控件仍主要来自本地枚举 | 下一步按 `fields/options/defaultValue` 生成动态字段控件，并把字段值统一写入 `QuickCreationCreateRequestDto.params`。 |
