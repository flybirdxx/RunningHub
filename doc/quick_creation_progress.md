...
| UI | `QuickCreateUiState.kt` / `QuickCreateScreenModel.kt` / `QuickCreateScreen.kt` | 新增历史加载状态、分页 state、历史列表 state、取消中 state、详情 state、项目列表 state、项目任务筛选 state、项目置顶中 state 和项目变更中 state；创作页在无当前结果/任务时展示最近创作，支持图片/视频预览、状态、分类和扣费金额摘要；历史区顶部展示项目横向筛选条和新建按钮，点击项目会加载项目内任务，点击“最近创作”会清除筛选，点击项目 chip 图钉会调用置顶/取消置顶，更多菜单可重命名/删除项目；历史列表可加载更多并去重追加；非终态历史任务会每 5 秒刷新当前已加载范围并显示取消入口；点击历史项按 `outputId` 加载详情弹窗；生成成功后刷新历史。 |
...
| 历史能力仍不完整 | 已能在创作页展示最近创作、项目横向筛选、项目内任务、项目置顶切换、项目创建/重命名/删除、加载更多历史、定时刷新非终态任务、取消非终态任务并打开详情弹窗；但项目详情 UI 尚未设计，且非空项目真实响应仍需抓包校正 | 下一步补项目详情 UI，并在有非空项目时校正 `project/tasks`、`project/pin`、`project/create/rename/delete/detail` 请求/响应结构。 |
