...
已完成：
- `handleTaskStatus(Queuing)` 现在会在更新任务状态前复用 `clearDraft()`，同步清掉 `uiState.draftData`、取消待执行草稿自动保存，并异步清理持久化草稿。
- 修复成功提交后只清持久化草稿、不清内存草稿入口的问题，避免任务结束后底部再次出现旧草稿提示条。
- 删除 `Queuing` 分支里直接调用 `settingsRepository.clearQuickCreateDraft()` 的重复路径，让恢复、丢弃、成功提交三条路径共享同一套草稿清理语义。
- 新增回归测试覆盖：已有旧草稿入口时应用灵感模板并提交，进入排队后 `uiState.hasDraft=false` 且持久化草稿为空。
...
- 真机仍需验证已有旧草稿入口时应用模板或直接生成，提交进入排队后草稿提示条不会在任务结束后重新出现。
- 本轮没有触发真实生成、`prepare/commit` 或新增扣费。
- 后续提交继续避开已有 `shared/src/commonMain/kotlin/com/runninghub/shared/data/repository/AuthRepositoryImpl.kt` 修改和未跟踪 `output/` 证据目录。
## 2026-06-18 失败任务替换运行中文案
...
已完成：
- `handleTaskStatus(Failed)` 现在会把 `statusText` 更新为服务端失败消息，避免任务先进入 `Running` 后失败时，中间状态区继续显示旧的“生成中...N%”文案。
- 测试 Fake repository 支持自定义图片任务状态流，可覆盖 `Running -> Failed` 这类多状态序列。
- 新增回归测试覆盖：图片任务先返回 `Running(progress=42)` 再返回 `Failed("render failed")` 时，`taskStatus=FAILED`、`error=render failed`、`statusText=render failed`。
...
仍未完成：
- 远端已同步；后续继续保持代码提交和文档提交交替推送。
- 真机仍需验证任务从运行中进入失败态时，中间区域不再显示旧进度文案。
- 本轮没有触发真实生成、`prepare/commit` 或新增扣费。
- 后续提交继续避开已有 `shared/src/commonMain/kotlin/com/runninghub/shared/data/repository/AuthRepositoryImpl.kt` 修改和未跟踪 `output/` 证据目录。
## 2026-06-18 清空成功结果时清理状态文案
...
已完成：
- `clearResults()` 现在会同时清空 `results`、把 `taskStatus` 置回 `IDLE`，并把 `statusText` 置为 `null`。
- 修复用户清空成功生成结果后，状态对象里仍残留“生成完成”等旧文案的问题。
- 新增回归测试覆盖：图片任务返回 `Success` 后调用 `clearResults()`，应同时得到 `taskStatus=IDLE`、`results=[]`、`statusText=null`。
...
- 修复旧行为：`LIST`/options 这类必填服务端动态字段若没有默认值，旧逻辑只校验文本和上传字段，可能继续进入正式 `generateImage()`，导致 prepare/commit 请求体缺少必填参数。
- 新增回归测试覆盖：必填 `stylePreset` options 字段为空时，不应提交图片生成请求，`taskStatus=IDLE`，错误文案为 `Style preset 不能为空`。
...
## 2026-06-18 空 prompt 拦截时清理提交状态文案
...
已完成：
- 图片和视频生成在 prompt 为空或超过限制时，回到 `IDLE` 的同时会清空 `statusText`。
- 修复旧行为：`generate()` 先写入 `statusText=正在提交任务...`，随后 `generateImage()` / `generateVideo()` 因 prompt 无效本地拦截时，只设置 `taskStatus=IDLE` 和错误文案，没有清理提交中文案，可能让状态区显示“正在提交任务...”残留。
- 新增回归测试覆盖：空图片 prompt 和空视频 prompt 都不应提交正式请求，`taskStatus=IDLE`、`statusText=null`、错误文案为 `请输入描述词`。
...
仍未完成：
- 真机仍需验证用户清空图片或视频 prompt 后点击生成，状态区不会残留“正在提交任务...”。
- 本轮没有触发真实生成、`prepare/commit` 或新增扣费。
- 后续提交继续避开已有 `shared/src/commonMain/kotlin/com/runninghub/shared/data/repository/AuthRepositoryImpl.kt` 修改和未跟踪 `output/` 证据目录。
...
- 修复旧行为：如果用户选择了可选参考素材且上传在点击生成前已经失败，旧流程会因为没有 pending 上传而跳过等待，并把失败素材静默忽略，继续提交 prompt-only 正式任务。
- 新增回归测试覆盖：全局参考图上传失败后点击生成，不应提交图片生成请求，`taskStatus=IDLE`、`statusText=null`，错误文案为 `素材上传失败: test.jpg`。
...
## 2026-06-18 价格预览失败后回退本地估算价

代码提交待本段文档提交后与实现交替推送到 `feature/kmp-refactoring`。

已完成：
- `scheduleFeePreview()` 的无可用预览请求分支改为复用当前 tab 的本地估算价 helper，避免图片/视频各自重复判断。
- 图片 fee-preview 失败分支现在会把 `estimatedCost` 回退到当前图片配置的本地估算价，同时保留 `feePreviewError`，让生成仍被“价格待确认”阻止。
- 视频 fee-preview 失败路径通过 `applyFeePreviewError()` 使用同一套本地估算价回退逻辑。
- 修复旧行为：用户先拿到一次服务端预览金额 `0.76`，随后下一次 fee-preview 失败时，状态里仍保留旧服务端金额，容易让按钮价格和“价格待确认”语义不一致。
- 新增回归测试覆盖：先完成图片服务端预览，再模拟下一次图片 fee-preview 失败，断言金额回退到 `imageConfig.estimatedCost`，并保留失败状态。
...
## 2026-06-18 价格预览未通过时不保留服务端金额

代码提交待本段文档提交后与实现交替推送到 `feature/kmp-refactoring`。

已完成：
- `applyFeePreview()` 在服务端预览通过时继续使用 `requiredCashAmount/requiredRhAmount` 写入 UI 金额。
- 当 `passed=false` 或 `insufficientType != null` 时，`estimatedCost` 改为回退当前 tab 的本地估算价，同时保留 `feePreviewError=余额不足或价格预览未通过`。
- 图片和视频未通过预览测试新增金额断言，确保不可提交状态不会继续持有服务端返回的不可用金额。
- 保留正常通过预览、预览失败回退本地估算价、未通过预览阻断提交的既有行为。
...
## 2026-06-18 视频非激活 child 上传字段素材不刷新价格预览
代码提交待本文档提交后与测试覆盖交替推送到 `feature/kmp-refactoring`。

已完成：
- 新增 `inactive child service upload field media does not refresh video fee preview` 回归测试。
- 新增 `removing inactive child service upload field media does not refresh video fee preview` 回归测试。
- 覆盖视频 tab 下 `creationMode=text` 时，绑定在 `visibleWhen creationMode=videoReference` 的 child `VIDEO_UPLOAD` 字段素材选择和移除都不应触发新的视频 fee-preview。
- 生产代码未改动；当前 active upload param key 过滤已经覆盖视频 inactive child 上传字段。
...
## 2026-06-18 项目列表分页加载更多

代码提交待本文档提交后与实现提交交替推送到 `feature/kmp-refactoring`。

已完成：
- `QuickCreateUiState` 新增 `projectsLoadingMore`，区分项目列表初次加载和后续分页加载。
- `QuickCreateScreenModel` 新增 `loadMoreQuickCreationProjects()`：当 `projectsHasMore=true` 且当前未加载时，请求下一页 `/task/quick-creation/project/list`，追加项目并按 `projectId` 去重。
- 项目横向条末尾在 `projectsHasMore=true` 时展示“加载更多”chip；点击后加载下一页，加载中展示 spinner 并禁用重复点击。
- 新增 `loading more projects appends next page` 回归测试，覆盖第一页 `hasNext=true`、第二页追加、`projectsPage/projectsHasMore/projectsLoadingMore` 状态更新。
