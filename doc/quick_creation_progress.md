...
| Phase 1 | 图片 G-2.0 MVP | 部分完成 | 55% | 默认 `all-power-image-g2` 已切到 Web v2 的 `fee-preview -> prepare -> commit -> list`；真实端到端扣费生成尚未在 App 内复测。 |
| Phase 2 | 页面结构重构 | 部分完成 | 70% | 页面已调整为顶部“创作/灵感”、中间滚动内容、底部固定输入面板；灵感区目前是本地占位数据。 |
| Phase 3 | 灵感接口接入 | 未完成 | 15% | API DTO/端点已预留，ScreenModel 尚未拉取真实 tags/templates。 |
...
| 测试 | `composeApp/src/commonTest/kotlin/com/runninghub/app/ui/feature/quickcreate/QuickCreateScreenModelTest.kt` | 增加创作/灵感模式切换测试，并补 Main dispatcher 测试配置。 |
...
| UI | `QuickCreateScreen.kt` | 改为顶部模式切换、中间内容区、底部固定输入区；灵感页加入占位列表。 |
...
| 模型和字段仍以本地枚举为主 | 还不是完整服务端驱动 | 下一步接入 `/api/qc/v2/models`，用服务端 `fields/options/defaultValue` 渲染调参。 |
| 灵感页为占位数据 | 只能验证布局，不能使用真实模板 | 接 `/task/quick-creation/inspiration/tags/templates/detail`。 |
