...
| Phase 5 | 历史/项目 | 部分完成 | 92% | `list/detail/cancel/project/list/project/tasks/project/pin` DTO/API 已可解析，domain repository 已公开历史分页、按 `outputId` 获取详情、按 `taskId` 取消任务、项目列表分页、项目任务分页和项目置顶切换；创作页中间区域已展示最近创作，可打开详情弹窗，已接入加载更多分页，会对非终态历史任务定时刷新，并可取消非终态任务；历史区顶部已展示项目筛选条，点击项目会加载项目内任务并可切回最近创作，项目 chip 可切换置顶状态；项目创建/重命名/删除/详情尚未接入。 |
...
| 测试 | `shared/src/commonTest/kotlin/com/runninghub/shared/data/repository/QuickCreateRepositoryImplHistoryTest.kt` | 覆盖 `/task/quick-creation/list` 历史分页映射、扣费字段、`apiRequestParams` 解析、输出尺寸解析、`/task/quick-creation/detail` 按 `outputId` 获取详情、`/task/quick-creation/cancel` 使用 URL 编码后的 `taskId` 请求体、`/task/quick-creation/project/list` 项目分页映射和请求体、`/task/quick-creation/project/tasks` 按 `projectId/page/size` 加载项目内任务，以及 `/task/quick-creation/project/pin` 按 `projectId/pin` 提交置顶状态。 |
| 测试 | `composeApp/src/commonTest/kotlin/com/runninghub/app/ui/feature/quickcreate/QuickCreateScreenModelTest.kt` | 覆盖 ScreenModel 初始化加载 quick-creation 历史和项目列表、选择项目后加载项目内任务、清除项目筛选后回到最近创作、加载更多追加下一页、非终态历史任务定时刷新到终态、取消历史任务后刷新列表、选择 output 后按 `outputId` 加载详情并写入 UI state，以及切换项目置顶时调用 repository 并更新本地项目 state。 |
...
| UI | `QuickCreateUiState.kt` / `QuickCreateScreenModel.kt` / `QuickCreateScreen.kt` | 新增历史加载状态、分页 state、历史列表 state、取消中 state、详情 state、项目列表 state、项目任务筛选 state 和项目置顶中 state；创作页在无当前结果/任务时展示最近创作，支持图片/视频预览、状态、分类和扣费金额摘要；历史区顶部展示项目横向筛选条，点击项目会加载项目内任务，点击“最近创作”会清除筛选，点击项目 chip 图钉会调用置顶/取消置顶并更新本地项目列表；历史列表可加载更多并去重追加；非终态历史任务会每 5 秒刷新当前已加载范围并显示取消入口；点击历史项按 `outputId` 加载详情弹窗；生成成功后刷新历史。 |
...
| 历史能力仍不完整 | 已能在创作页展示最近创作、项目横向筛选、项目内任务、项目置顶切换、加载更多历史、定时刷新非终态任务、取消非终态任务并打开详情弹窗；但项目创建/重命名/删除/详情等管理动作仍未接入，且非空项目真实响应仍需抓包校正 | 下一步补项目创建/重命名/删除/详情交互，并在有非空项目时校正 `project/tasks` 与 `project/pin` 请求/响应结构。 |
