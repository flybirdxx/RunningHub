...
- 项目列表：已公开 `listQuickCreationProjects(page,size)`，使用 `/task/quick-creation/project/list`，请求体为 `{"page":1,"size":20}`，响应按 `records/size/current/total/pages/hasNext/hasPrevious/nextCursor` 映射为 domain 分页模型；`QuickCreateScreenModel` 初始化会加载项目列表，历史区顶部会展示项目横向列表。当前账号抓包返回空列表，项目任务和项目管理动作尚未接入。
...
6. 补项目任务和管理动作：`project/create/rename/delete/pin/detail/tasks`；`project/list` 已进入 ScreenModel 和历史区 UI。
